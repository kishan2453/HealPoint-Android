...
function detectMetroHost(): string {
  ...
  const manifest2ExpoGoDebuggerHost = (
    (constants.manifest2 as { extra?: { expoGo?: { debuggerHost?: unknown } } } | null | undefined)
...
export function resolveApiUrl(): string {
  ...
  // No env override and no Metro host information (e.g. a static export
  // without a dev server). Keep a labelled placeholder so misconfigurations
  // surface as a clear development message instead of a silent 404/ECONN.
  return `http://${PLACEHOLDER_HOST}:${API_PORT}${API_BASE_PATH}`;
}
...
export const GOOGLE_WEB_CLIENT_ID =
  (process.env.EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID || process.env.EXPO_PUBLIC_GOOGLE_CLIENT_ID || '').trim();
...
export function isApiUrlConfigured(): boolean {
  return !API_URL.includes(PLACEHOLDER_HOST);
}
