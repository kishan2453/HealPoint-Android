        ...
        <AuthProvider>
          <FavoritesProvider>
            <Stack screenOptions={{ headerShown: false }}>
              ...
              <Stack.Screen name="payment/[appointmentId]" options={{ title: 'Secure payment' }} />
              <Stack.Screen name="appointment/reschedule/[id]" options={{ presentation: 'modal', title: 'Reschedule appointment' }} />
              <Stack.Screen name="booking/[doctorId]" options={{ presentation: 'modal', title: 'Book appointment' }} />
              <Stack.Screen name="profile/edit" options={{ presentation: 'modal', title: 'Edit profile' }} />
              <Stack.Screen name="profile/change-password" options={{ presentation: 'modal', title: 'Change password' }} />
              <Stack.Screen name="notification/index" options={{ presentation: 'modal', title: 'Notifications' }} />
...
export function ErrorBoundary({ error, retry }: StartupErrorBoundaryProps) {
  ...
  return (
    <View style={styles.errorContainer}>
      <ScrollView style={styles.errorScroll} contentContainerStyle={{ padding: 24 }} testID="healpoint_startup_error">
        <Text style={styles.errorTitle}>
          HealPoint failed to start
        </Text>
      ...
      <Pressable onPress={retry} style={styles.retryButton}>
        {({ hovered, pressed }) => (
          <Text style={[styles.retryText, (hovered || pressed) && styles.retryTextActive]}>
            Retry
          </Text>
        )}
      </Pressable>
...
const styles = StyleSheet.create({
  ...
  errorStack: {
    color: '#1B2A28',
    fontSize: 12,
    fontFamily: Platform.select({ default: 'monospace', ios: 'monospace', android: 'monospace' }),
    lineHeight: 16,
    marginBottom: 16,
  },
  errorHint: {
