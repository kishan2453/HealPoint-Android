...
import type { Subscription, SubscriptionListSort, SubscriptionOverview, SubscriptionPlan } from '@/types';
...
export default function SuperAdminSubscriptionsScreen() {
  ...
  const maxSubscribers = Math.max(1, ...plans.map((item) => item.subscriberCount || 0));
  const renewals = useMemo(() => (overview?.upcomingRenewals || []).slice(0, 5), [overview]);
            ...
            <View style={styles.headerWrap}>
              <Card style={styles.heroCard}>
                <View style={styles.heroTexts}>
                  ...
                  <Text style={styles.heroTitle}>Subscription control panel</Text>
                  <Text style={styles.heroSubtitle}>
                    {total} hospital subscription(s) · {overview?.activeSubscriptions ?? 0} active ·{' '}
                    {formatINR(overview?.revenue)} recorded revenue
                  </Text>
                </View>
                <View style={styles.heroIcon}>
                  <Ionicons name="card-outline" size={26} color={Palette.primaryDark} />
                </View>
              </Card>
              ...
              <View style={styles.grid}>
                ...
                <StatCard label="Suspended" value={overview?.suspendedCount ?? '—'} icon="pause-circle-outline" accent="#7B61FF" />
                <StatCard label="Past due" value={overview?.pastDueCount ?? '—'} icon="alert-circle-outline" accent="#E89A3C" />
                <StatCard label="Expiring ≤30d" value={overview?.expiringCount ?? '—'} icon="alarm-outline" accent="#D9435B" />
              </View>
                ...
                <Card style={styles.revenueCard}>
                  <View style={styles.cardHeader}>
                    <View style={styles.cardHeaderTexts}>
                      ...
                      <Text style={styles.cardHeaderSub}>Collected from stored subscription payments</Text>
                    </View>
                    <View style={styles.cardHeaderIcon}>
                      <Ionicons name="wallet-outline" size={20} color={Palette.success} />
                    </View>
                  </View>
                  <View style={styles.revenueRow}>
                    <View style={styles.revenueItem}>
                      <Text style={styles.revenueLabel}>Total revenue</Text>
                      <Text style={styles.revenueValue}>{formatINR(overview?.revenue)}</Text>
                    </View>
                    <View style={styles.revenueItem}>
                      <Text style={styles.revenueLabel}>Paid</Text>
                      <Text style={styles.revenueValue}>{formatINR(overview?.paidRevenue)}</Text>
                    </View>
                    <View style={styles.revenueItem}>
                      <Text style={styles.revenueLabel}>Active subscriptions</Text>
                      <Text style={styles.revenueValue}>{overview?.activeSubscriptions ?? '—'}</Text>
                    </View>
                  </View>
                </Card>
                <Card style={styles.renewalCard}>
                  ...
                  {plans.length === 0 ? (
                    ...
                    plans.map((planItem) => {
                      ...
                      const widthPct = Math.min(100, Math.round((count / maxSubscribers) * 100));
                      ...
                      return (
                        <Pressable
                          ...
                          onPress={() => setPlan(activeFilter ? 'all' : planItem.key)}
                          style={({ pressed }) => [styles.planRow, pressed && styles.planRowPressed]}
                        >
                          <View style={styles.planTexts}>
                            <View style={styles.planNameRow}>
                              <Text style={styles.planName} numberOfLines={1}>{planItem.name}</Text>
                              <Badge
                                label={activeFilter ? 'Filtering' : planItem.isActive ? 'Active' : 'Inactive'}
                                variant={activeFilter ? 'primary' : planItem.isActive ? 'success' : 'neutral'}
                              />
                            </View>
                          ...
                          <View style={styles.planCount}>
                            ...
                            <Text style={styles.planCountLabel}>hospital(s)</Text>
                          </View>
                        </Pressable>
                      );
                    })
              ...
              <View style={styles.searchWrap}>
                <SearchBar value={query} onChangeText={setQuery} placeholder="Search hospital or plan..." />
              </View>
              <FilterChips options={STATUS_FILTERS} selected={status} onSelect={setStatus} />
              <FilterChips options={planOptions} selected={plan} onSelect={setPlan} />
              <FilterChips options={[...SORTS]} selected={sort} onSelect={(value) => setSort(value as SubscriptionListSort)} />
            </View>
          }
          ListEmptyComponent={
            !loading ? (
...
const styles = StyleSheet.create({
  ...
  heroEyebrow: { ...Typography.caption, color: Palette.primaryDark, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.4 },
  ...
  revenueLabel: { ...Typography.caption, color: Palette.textMuted, textTransform: 'uppercase', letterSpacing: 0.3 },
  ...
  barTrack: { height: 6, borderRadius: 3, backgroundColor: Palette.background, overflow: 'hidden' },
