...
export default function SuperAdminPlansScreen() {
  ...
  const [pendingStatus, setPendingStatus] = useState<SubscriptionPlan | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const sorted = [...(res.plans || [])].sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0));
      setPlans(sorted);
      setSelected((prev) =>
  ...
  const visiblePlans = useMemo(() => {
    ...
    let list = plans;
    if (needle) list = list.filter((plan) => plan.name.toLowerCase().includes(needle) || plan.key.toLowerCase().includes(needle));
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.heroCard}>
          ...
          <View style={styles.heroIcon}>
            <Ionicons name="pricetags-outline" size={24} color={Palette.primaryDark} />
          </View>
        </View>

        {notice ? <FormMessage type="success" message={notice} /> : null}
        {actionError && !editorOpen ? <FormMessage type="error" message={actionError} /> : null}

        <View style={styles.searchWrap}>
          <SearchBar value={query} onChangeText={setQuery} placeholder="Search plan name..." />
        </View>
        ...
        {visiblePlans.length === 0 ? (
          <EmptyState
            ...
            action={
              <Button title="Create plan" variant="secondary" icon="add-circle-outline" onPress={() => openEditor(null)} />
            }
          />
        ) : (
      ...
      <ConfirmDialog
        ...
        title={pendingStatus?.isActive ? `Deactivate ${pendingStatus?.name || 'plan'}?` : `Activate ${pendingStatus?.name || 'plan'}?`}
        message={
...
function PlanCard({ plan, style, onOpen, onToggle }: PlanCardProps) {
  return (
    <Card style={[styles.planCard, style]}>
      <Pressable onPress={onOpen} accessibilityRole="button" accessibilityLabel={`Open ${plan.name} details`}>
        <Image source={{ uri: getPlanImage(plan) }} style={styles.planImage} contentFit="cover" transition={250} />
      ...
      <View style={styles.planBody}>
        <View style={styles.planTitleRow}>
          <View style={styles.planTitleWrap}>
            ...
            <Text style={styles.planKey}>{planKeyLabel(plan.key)} · {plan.subscriberCount ?? 0} hospital(s)</Text>
          </View>
        ...
        <View style={styles.priceRow}>
          <Text style={styles.price}>{formatINR(plan.monthlyPrice)}<Text style={styles.pricePeriod}>/month</Text></Text>
          <Text style={styles.priceDivider}>·</Text>
          <Text style={styles.price}>{formatINR(plan.yearlyPrice)}<Text style={styles.pricePeriod}>/year</Text></Text>
        </View>
        ...
        <View style={styles.planActions}>
          <Button title="View" variant="secondary" fullWidth={false} style={styles.planActionBtn} icon="eye-outline" onPress={onOpen} />
          <Button
            title={plan.isActive ? 'Deactivate' : 'Activate'}
            variant={plan.isActive ? 'outline' : 'primary'}
            fullWidth={false}
            style={styles.planActionBtn}
            icon={plan.isActive ? 'pause-circle-outline' : 'play-circle-outline'}
            onPress={onToggle}
          />
        </View>
      </View>
    ...
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.backdrop}>
        ...
        <Card style={styles.detailsCard}>
          <Image source={{ uri: getPlanImage(plan) }} style={styles.detailsImage} contentFit="cover" transition={250} />
          <View style={styles.detailsBody}>
            <View style={styles.detailsHeader}>
              ...
              <Pressable onPress={onClose} style={styles.closeButton} accessibilityLabel="Close" hitSlop={8}>
            ...
            <View style={styles.badgeRow}>
              ...
              {plan.trialDays ? <Badge label={`${plan.trialDays}-day trial`} variant="warning" /> : null}
            </View>
            ...
            <View style={styles.detailGrid}>
              <View style={styles.detailCol}>
                ...
                <Text style={styles.detailValue}>{formatINR(plan.monthlyPrice)}</Text>
              </View>
              <View style={styles.detailCol}>
                <Text style={styles.detailLabel}>Yearly</Text>
                <Text style={styles.detailValue}>{formatINR(plan.yearlyPrice)}</Text>
              </View>
              <View style={styles.detailCol}>
            ...
            <View style={styles.metaRow}>
              <Text style={styles.metaText}>Created {formatISODate(plan.createdAt)}</Text>
              <Text style={styles.metaText}>Updated {formatISODate(plan.updatedAt)}</Text>
            </View>

            <Button title="Edit plan" variant="secondary" icon="create-outline" onPress={onEdit} />
          </View>
        </Card>
      </View>
    </Modal>
...
function PlanEditorModal({
  ...
  const previewUri = getPlanImage({ key: editing?.key, imageUrl: form.imageUrl || undefined });
  ...
  const pickImage = async () => {
    ...
    try {
      ...
      if (!permission.granted) {
        setImageError('Photo library permission is required to pick a plan image.');
        return;
      }
      ...
      if (asset.base64) {
        ...
        setForm((prev) => ({ ...prev, imageUrl: `data:${mime};base64,${asset.base64}` }));
        setImageError('');
      } else {
        setImageError('Could not read the selected image. Try again or paste an image URL instead.');
      }
    } catch {
      setImageError('Image picker is unavailable here. Paste an image URL instead.');
    }
  };
      ...
      <View style={styles.backdrop}>
        <View style={styles.modalCard}>
          ...
          <ScrollView style={styles.form} showsVerticalScrollIndicator={false}>
            ...
            <Pressable onPress={pickImage} accessibilityRole="button" accessibilityLabel="Choose plan image">
            ...
            <Text style={styles.hint}>Tap the image to pick from your library, or paste a healthcare image URL below.</Text>
            {imageError ? <FormMessage type="warning" message={imageError} /> : null}
            <TextInput
              style={styles.input}
              value={form.imageUrl}
...
<Text style={styles.label}>Plan key {editing ? `(${editing.key})` : ''} *</Text>
            ...
            <View style={styles.twoCol}>
              ...
              <View style={styles.twoColItem}>
                ...
                <TextInput style={styles.input} value={form.sortOrder} onChangeText={(text) => setForm((prev) => ({ ...prev, sortOrder: text }))} keyboardType="numeric" placeholder="1" />
              </View>
            </View>
          ...
          <View style={styles.modalActions}>
            <Button title="Cancel" variant="outline" fullWidth={false} style={styles.modalActionBtn} onPress={onCancel} disabled={saving} />
            <Button title={editing ? 'Save changes' : 'Create plan'} fullWidth={false} style={styles.modalActionBtn} onPress={onSave} loading={saving} />
          </View>
        </View>
      </View>
    </Modal>
...
const styles = StyleSheet.create({
  ...
  planImage: { width: '100%', height: 150, backgroundColor: Palette.primaryLight },
  planImageOverlay: { position: 'absolute', top: Spacing.sm, right: Spacing.sm },
  ...
  noFeatures: { ...Typography.caption, color: Palette.textMuted, fontStyle: 'italic' },
  ...
  detailsImage: { width: '100%', height: 200, backgroundColor: Palette.primaryLight },
  ...
  removeImageText: { ...Typography.bodySmall, color: Palette.error, fontWeight: '600' },
