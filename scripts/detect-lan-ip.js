...
function detectLanIPv4() {
  ...
  const virtualName = /virtual|vethernet|wsl|hyper|loopback|docker|vmware|virtualbox|tailscale|zerotier|tap|tun|cisco|vpn/i;
  ...
  for (const key of Object.keys(networks)) {
    ...
    for (const net of networks[key] || []) {
      ...
      if (/^172\.(1[6-9]|2\d|3[01])\./.test(ip) && /vethernet|wsl|hyper/i.test(key)) continue;
...
function run() {
  ...
  if (currentValue && !force) {
    console.log(`[detect:ip] EXPO_PUBLIC_API_URL is already set to: ${currentValue}`);
    console.log('[detect:ip] Re-run with --force to overwrite it with a fresh detection.');
    process.exit(0);
  }
  ...
  if (!ip) {
    ...
    console.error('            Pass it manually, e.g.:  node scripts/detect-lan-ip.js 192.168.1.34');
    process.exit(1);
  }
